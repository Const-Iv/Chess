...
- Для шахматного продукта нельзя превращать приложение в справочник ходов без объяснения дебютных принципов, целей позиции и планов до миттельшпиля.
- Нельзя выдавать неподтвержденные названия вариантов, AI-generated opening names или "лучшие ходы" как факт; нужен source или manual verification note.
- До echo-test legal move parsing + manually verified opening-map нельзя начинать feature/refactor/behavior-change work по шахматной логике.
...
- Новый downstream-проект сначала проходит Project Intake Gate по `plans/_project_intake_template.md`: миссия, видение, цель, целевая аудитория, `JTBD`, ограничения, сценарии, метрики, core/adapters/profiles boundary, stack/runtime choices, echo-testing для unknown root technology, integration/review path, QA/release choices, agent/eval ownership, source-of-truth ownership, rules/memory ownership и applicable capability decisions должны быть заполнены и явно согласованы owner'ом до первой feature/refactor/behavior-change реализации в соответствующей зоне.
...
- После `worktree-finish` или `task:merge:main`, которые затрагивают repo-defined deploy profile, ассистент должен проверить deploy evidence: `publishStatus`, source branch/SHA, target branch, provider/deployment URL/status если доступен, production URL smoke when appropriate. Если подтвержден только push в deployment branch, это deployment trigger evidence, а не proof of completed deploy.
...
- Для user-visible UI behavior change или UI bugfix `qa:agent`, typecheck, unit tests, HTTP reachability и code review недостаточны без browser oracle evidence, если реальный интерфейс можно запустить.
